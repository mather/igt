...
- 開始・終了を同じマーカー行 `### igt: <TemplateName> ###` で挟む
- `igt:` プレフィックスにより、テンプレート内容との衝突を回避（例: AL.gitignore内の `### AL ###` との衝突防止）
...
1. `.gitignore` 全体をパースし、`### igt: <name> ###` 〜 次の同名 `### igt: <name> ###` までを1セクションとして認識
